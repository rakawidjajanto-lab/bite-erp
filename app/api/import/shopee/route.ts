...
import { parseCSVText, normalizeShopeeRow } from "@/lib/import/platform-normalizer";
...
export async function POST(req: Request) {
  ...
  let platform = await prisma.platform.findUnique({ where: { name: "Shopee" } });
  if (!platform) {
    platform = await prisma.platform.create({ data: { name: "Shopee" } });
  }
  ...
  for (const row of rows) {
    const normalized = normalizeShopeeRow(row);
    ...
    try {
      ...
      const tx = await prisma.transaction.create({
        data: {
          ...
          description: `Shopee order #${normalized.externalOrderId}`,
          category: "SALES",
